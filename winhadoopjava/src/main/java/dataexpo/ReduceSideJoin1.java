...
public class ReduceSideJoin1 {
	public static void main(String[] args) throws IllegalArgumentException, IOException, ClassNotFoundException, InterruptedException {
		...
		job.setJarByClass(ReduceSideJoin1.class); // 작업 클래스 설정
		job.setPartitionerClass(TaggedGroupKeyPartitioner.class);
		job.setGroupingComparatorClass(TaggedGroupKeyComparator.class);
		job.setSortComparatorClass(TaggedGroupComparator.class); // 새로운 정렬 방식을 지정 (동적으로 정렬방식을 지정할 수 있다.)
